...
func TestCollisionRegistrySpellAward(t *testing.T) {
	...
	defer func() {
		collisionRegistryCapture(t, "collision-registry-spell-award", rows)
	}()
			...
			for _, quest := range []bool{false, true} {
				for _, player := range []bool{false, true} {
					...
					t.Run(name, func(t *testing.T) {
						...
						legacy.PortTestRegisteredCollision(a, target, nil, "AwardSpellCollide")
						...
						if objectXferGetWord(p, 3696+4*id) != want {
							t.Fatalf("award level%d want%d success%t", objectXferGetWord(p, 3696+4*id), want, awarded)
						}
						var sounds []int
						...
						rows = append(rows, struct {
							...
							Sounds []int
							Queue  int
						}{name, objectXferGetWord(p, 3696+4*id), sounds, len(o.state().Nodes)})
					})
				}
			}
		}
	}
	legacy.PortTestRegisteredCollision(a, nil, nil, "AwardSpellCollide")
}
