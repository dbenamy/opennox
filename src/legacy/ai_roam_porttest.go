...
type PortTestRoamSpec struct {
	...
	Callbacks                  *PortTestAICallbackSpec
...
type PortTestRoamResult struct {
	Callbacks          *PortTestAICallbackResult   `json:",omitempty"`
...
func PortTestRoam(specs []PortTestRoamSpec) []PortTestRoamResult {
	...
	for _, sp := range specs {
		if sp.Callbacks != nil {
			defer portTestAICallbackEnvironment(proxy)()
			break
		}
	}
	for _, sp := range specs {
	...
	for _, sp := range specs {
		if sp.Callbacks != nil && sp.Callbacks.Penalty != nil {
			defer portTestPenaltyEnvironment(proxy)()
			break
		}
	}
	for _, sp := range specs {
		if sp.Callbacks != nil && sp.Callbacks.Death != nil {
			defer portTestDeathEnvironment(proxy)()
			break
		}
	}
	...
	for _, sp := range specs {
		if sp.Callbacks != nil && sp.Callbacks.Generator != nil && sp.Callbacks.Generator.SpawnPolicy != nil {
			defer portTestSpawnPolicyEnvironment(proxy)()
			break
		}
	}

	for _, sp := range specs {
		if sp.Callbacks != nil && sp.Callbacks.Shop != nil {
			defer portTestShopEnvironment(proxy)()
			break
		}
	}
	...
	for _, sp := range specs {
		...
		if sp.Callbacks != nil {
			portTestAICallbackPrepare(proxy, obj, sp.Callbacks)
		}
		if sp.Path != nil {
		...
		var callbackResult *PortTestAICallbackResult
		...
		switch sp.Op {
		case 15:
			rv := portTestAICallbackCall(proxy, obj, sp.Callbacks)
			callbackResult = portTestAICallbackTrace(proxy, rv, normalize)
			copy(beforeD[8:len(beforeD)-8], db[8:len(db)-8])
			spellResult = portTestAISpellTrace(proxy, obj, 0, normalize)
