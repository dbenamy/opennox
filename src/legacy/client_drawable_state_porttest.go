...
func PortTestDrawableStream(stream int, input []byte, pos [2]int32) (int32, [2]int32) {
	...
	var ret int32
	if stream == 1 {
		ret = int32(C.nox_xxx_netCliProcUpdateStream_494A60((*C.uchar)(unsafe.Pointer(&packet[0])), 31, (*C.uint32_t)(unsafe.Pointer(&coords[0]))))
	} else {
		ret = int32(uintptr(unsafe.Pointer(C.nox_xxx_netCliUpdateStream2_494C30((*C.uchar)(unsafe.Pointer(&packet[0])), 31, (*C.int)(unsafe.Pointer(&coords[0]))))))
	}
