...
func PortTestSecretWalls(ids []uint16, ops []PortTestSecretOperation) (out []PortTestSecretResult) {
	...
	for i, id := range ids {
		...
		// The overwritten next word is raw data until insertion links the node.
		w[0] = 0x13572468 + uint32(i)
		w[1] = uint32(i + 1)
		w[2] = 0x12345678
	...
	for _, op := range ops {
		...
		case 0:
			...
			r.Return = identify(unsafe.Pointer(worldSecretInsert(nodes[op.ID])), nodes)
			linked[op.ID] = true
		case 1:
			r.Return = identify(unsafe.Pointer(worldSecretRemove(nodes[op.ID])), nodes)
			if r.Return >= 0 {
				live[r.Return] = false
				linked[r.Return] = false
			}
		case 2:
			r.Return = identify(worldSecretFind(int16(op.ID)).C(), walls)
		case 3:
			r.Return = identify(unsafe.Pointer(worldSecretClear()), nodes)
			for i := range linked {
				if linked[i] {
					live[i] = false
					linked[i] = false
		...
		case 4:
			r.Return = identify(worldSecretNext(nil), nodes)
		}
		p := worldSecretHead
		for p != nil {
			i := identify(p, nodes)
