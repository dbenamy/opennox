...
func TestWorldCollisionsExitAdmission(t *testing.T) {
	...
	defer func() {
		spellbookCapture(t, "world-collisions-exit-admission", rows, "1ea73cdff61c3f25fbd3abb773a7b8fff5372bfb3862df2f915002985907cb80")
	}()
		...
		for _, path := range []string{"", "WorldTest.map", "WorldTest.map:Entry"} {
			...
			t.Run(name, func(t *testing.T) {
				...
				*o.globals["warpOpen"] = 0
				...
				legacy.PortTestWorldCollision(7, a, target, nil)
	...
	if legacy.PortTestWorldCollision(3, nil, nil, nil) != 0 {
		t.Fatal("pending map buffer")
	}
}

func worldCollisionExitGlobals(t *testing.T, o *worldCollisionOwner) map[string]*uint32 {
	words, restore := legacy.PortTestBookWords()
	t.Cleanup(restore)
	oldBusy, oldRetry, oldDelay, oldSaving := dword_5d4594_1563080, dword_5d4594_1563092, dword_5d4594_1563088, dword_5d4594_1563064
	t.Cleanup(func() {
		dword_5d4594_1563080 = oldBusy
		dword_5d4594_1563092 = oldRetry
		dword_5d4594_1563088 = oldDelay
		dword_5d4594_1563064 = oldSaving
	})
	oldSwitch := legacy.Get_dword_5d4594_1548524()
	t.Cleanup(func() { legacy.Set_dword_5d4594_1548524(oldSwitch) })
	for _, r := range []struct {
		base uintptr
		off  uintptr
		size int
	}{{0x5D4594, 2598188, 80}, {0x85B3FC, 36, 80}, {0x5D4594, 1523072, 4}, {0x973F18, 2408, 1464}, {0x5D4594, 1556108, 4}, {0x5D4594, 1556124, 4}, {0x587000, 4660, 4}, {0x5D4594, 3468, 8}} {
		raw := unsafe.Slice((*byte)(memmap.PtrOff(r.base, r.off)), r.size)
		old := bytes.Clone(raw)
...
func TestWorldCollisionsQuestExit(t *testing.T) {
	var rows []struct {
		Name                              string
	...
	defer func() {
		spellbookCapture(t, "world-collisions-quest-exit", rows, "82478c13ff1a911963f5cae944622e47690f2792bc02210186a321079d2448c5")
	}()
			...
			for _, allReady := range []bool{false, true} {
				...
				t.Run(name, func(t *testing.T) {
					...
					*o.globals["warpOpen"] = 2
					...
					legacy.PortTestWorldCollision(7, a, b, nil)
					after := platformTicks()
...
func TestWorldCollisionsCoopExitSave(t *testing.T) {
	o := newWorldCollisionOwner(t)
	words := worldCollisionExitGlobals(t, o)
	*words["dword_5d4594_1047520"] = 0
	...
	spellbookCapture(t, "world-collisions-coop-exit-save", rows, "1f0bc07575e272375bd4c0b2638cd38d3b7d6b030f744f38224870c1cba1abb7")
}

func TestWorldCollisionsExitGlyphCleanup(t *testing.T) {
	var rows []struct {
		Name    string
		Traps   byte
		Deleted [3]bool
	...
	defer func() {
		spellbookCapture(t, "world-collisions-exit-glyphs", rows, "2333be7981f9345ab7a40a408a745706355e6eacfe75fde09a3937e28c305ee2")
	}()
