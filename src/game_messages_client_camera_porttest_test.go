...
func TestGameMessageClientObjectCamera(t *testing.T) {
				...
				for same := 0; same < 2; same++ {
					for exists := 0; exists < 2; exists++ {
						for _, frame := range []uint32{0, 100, 0xffffffff} {
							...
							inputKeyTimeoutsOld = map[byte]uint32{9: timeout}
							...
							data := []byte{48, byte(code), byte(code >> 8), byte(typ), 0, 0xff, 0xff, 0, 0x80, 0xf9, 17}
							...
							calls = 0
							camera = image.Pt(-1, -1)
							n := legacy.Nox_xxx_netOnPacketRecvCli_48EA70_switch(0, netmsg.Op(48), data)
							wantCalls := 0
							wantCamera := image.Pt(-1, -1)
							if on != 0 && int(code) == local && same == 0 {
								wantCalls = 1
								wantCamera = image.Pt(65535, 32768)
							}
							if n != 11 || !bytes.Equal(data, want) || calls != wantCalls || camera != wantCamera || len(inputKeyTimeoutsOld) != 1 || inputKeyTimeoutsOld[9] != timeout || c.Objs.LoadError != (on != 0 && exists == 0) || c.Objs.Count != exists {
	...
	gameMessageCapture(t, "game-client-object-camera", rows)
}
