...
func TestWorldGeometryWallGrid(t *testing.T) {
	...
	for _, op := range []string{"circle-wall", "box-wall", "box-walls"} {
				...
				for _, window := range []bool{false, true} {
					for _, offset := range [][2]float32{{0, 0}, {11.5, 11.5}, {23, 23}, {0, 23}, {23, 0}, {-5, 11.5}, {28, 11.5}, {11.5, -5}, {11.5, 28}, {100, 100}} {
						for variant := 0; variant < 4; variant++ {
							...
							reset()
							worldGeometryResetObject(a, 1001, 138+offset[0], 92+offset[1], op != "circle-wall")
							...
							// Keep box shape storage valid for each variant.
							if op != "circle-wall" && variant == 1 {
								a.Shape.Box.W = 8
								a.Shape.Box.H = 12
								a.Shape.Box.Calc()
							}
							...
							scratch[0] = 6
							scratch[1] = 4
							rv := legacy.PortTestWorldGeometryPhysics(op, a, nil, &scratch, 0, 0)
							h := hits(ids)
							if len(h) > 0 {
								nonzero[op]++
							}
	...
	spellbookCapture(t, "world-geometry-wall-grid", rows, "2830cda1a94de825a6d1c22621c6dd8b6300c6a477eff25acf0f74ec5f9d2761")
}
