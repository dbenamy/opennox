...
// This sibling keeps TestPlayerDeathLifecycle's setup and assertions, but
// reaches PlayerDie through the registered Death pointer and projectileDie.
func TestPlayerDeathLifecycleRegistered(t *testing.T) {
	type record struct {
		Name    string
		Flags   uint32
		State   byte
		...
		for _, female := range []byte{0, 1} {
			...
			t.Run(name, func(t *testing.T) {
				...
				*(*byte)(unsafe.Add(pl.C(), 2252)) = female
				...
				legacy.PortTestDeathRegisteredProjectile(u, "PlayerDie")
	...
	spellbookCapture(t, "player-death-lifecycle-registry", rows, "1b3e763016a6be377a0956f0b45a2ab8516dcd2951621b9ee9502267c05f829f")
}
