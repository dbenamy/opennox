...
func TestProtectionAddABI(t *testing.T) {
	gen := rand.New(rand.NewSource(0x56f920))
	ids := []uint32{0, 657757278, 657757279, 657757280, 0x7fffffff, 0x80000000, 0xffffffff}
	deltas := []uint32{0, 1, 0xff, 0x100, 0x7fff, 0x8000, 0x8001, 0xffff, 0x10000, 0x7fffffff, 0x80000000, 0xffffffff}
	...
	for trial := 0; trial < 500; trial++ {
		...
		for mode := 0; mode < 4; mode++ {
			got := legacy.PortTestProtectionAdd(initial, key, sum, sequence, swapCount, rekeyCount, frame, floatSeed, seed, mode, id, value)
			...
			if int32(id) >= 657757279 {
				...
				if found >= 0 {
					// Widened signed arithmetic is an independent oracle for the
					// production uint32 addition and its modulo reduction.
					delta := int64(int32(value))
					if mode == 1 || mode == 3 {
						delta = int64(int16(value))
					}
					if mode == 2 {
						delta = int64(value & 0xff)
					}
					values[found][1] = uint32(int64(values[found][1]) + delta)
			...
			if (mode != 3 && got.Result != wantResult) || got.Key != wantKey || got.Sum != wantSum || got.Sequence != sequence || got.Count != uint16(len(values)) || got.SwapCount != wantSwaps || got.RekeyCount != wantRekeys || got.LogicIndex != rng.Index() || got.OtherIndex != other.Index() || got.FloatState != wantFloat || got.FloatRange != wantRange || !got.NodesAndLinksStable || !reflect.DeepEqual(got.Values, values) {
