...
func TestCollisionRegistryWorldDoorKeys(t *testing.T) {
	...
	defer func() {
		collisionRegistryCapture(t, "collision-registry-world-door-keys", rows)
	}()
				...
				for _, adjacent := range []bool{false, true} {
					...
					t.Run(name, func(t *testing.T) {
						...
						*(*byte)(unsafe.Add(near.UpdateData, 1)) = byte(lock)
						...
						collisionRegistryWorld(1, a, b, nil)
...
func TestCollisionRegistryWorldDoorMagic(t *testing.T) {
	o := newWorldCollisionOwner(t)
	a := newObjectXferSimple(t, o.s)
	b := &o.units[1]
	data := o.record(t, 64)
	...
	defer func() {
		collisionRegistryCapture(t, "collision-registry-world-door-magic", rows)
	}()
...
func TestCollisionRegistryWorldDoorMissingKey(t *testing.T) {
	o := newWorldCollisionOwner(t)
	a := newObjectXferSimple(t, o.s)
	b := &o.units[1]
	data := o.record(t, 64)
	...
	collisionRegistryCapture(t, "collision-registry-world-door-missing-key", rows)
}
