...
func TestWorldGeometryGate(t *testing.T) {
		...
		for _, offset := range [][2]float32{{0, 0}, {5, 0}, {-5, 0}, {0, 5}, {0, -5}, {15, 15}, {-15, -15}, {50, 50}} {
			for mode := 0; mode < 2; mode++ {
				for variant := 0; variant < 8; variant++ {
					...
					worldGeometryResetObject(b, 1002, 100+offset[0], 100+offset[1], true)
					...
					legacy.PortTestWorldGeometryPhysics("gate-box", a, b, &scratch, int32(mode), 0)
					h := hits(ids)
					q := queues(ids)
					if len(h) > 0 {
						contacts++
	...
	spellbookCapture(t, "world-geometry-gate", rows, "7ac58ce0a6950071bcabe059ab109874feeadbda67d91c3ef81c93cee5b5780c")
}
