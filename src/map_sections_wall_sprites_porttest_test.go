...
func TestMapSectionsWallVersionTwoSprites(t *testing.T) {
	...
	var wants []expected
	for _, version := range []uint32{2} {
		for _, generation := range []bool{false, true} {
			for _, load := range []uint32{0, 1, 2, 3} {
				for _, material := range []byte{0, 1, 2} {
					for _, direction := range []byte{0, 1, 11} {
						for _, variation := range []byte{0} {
							for _, point := range [][2]int{{6, 10}, {7, 11}, {8, 10}} {
								wire := binary.LittleEndian.AppendUint16(nil, uint16(version))
								for _, v := range []uint32{uint32(point[0]), uint32(point[1]), 0, 0} {
									wire = binary.LittleEndian.AppendUint32(wire, v)
								}
								wire = append(wire, direction|128)
								if version >= 2 {
									wire = append(wire, material)
								}
								if version >= 4 {
									wire = append(wire, 173)
								}
								if version >= 5 {
								...
								sp := legacy.PortTestMapSectionSpec{Name: fmt.Sprintf("v%d/gen%v/load%d/tile%d/dir%d/var%d/point%v", version, generation, load, material, direction, variation, point), Flags: flags, WallSprites: load&2 != 0,
									Paint: legacy.PortTestPaintSpec{Seed: 43, Globals: map[string]legacy.PortTestMapRoomArg{"section-magic-wall": {Value: 2}, "section-wall-load-flags": {Value: load & 1}, "section-secret-index": {Value: 19}, "section-breakable-index": {Value: 23}}, Actions: []legacy.PortTestPaintAction{{Op: 0}}},
	...
	for i, r := range out {
		t.Run(r.Name, func(t *testing.T) {
			...
			variation := byte(0)
			if want.load&2 != 0 && want.direction <= 1 {
				variation = byte(want.x % 3)
			}
	...
	spellbookCapture(t, "map-sections-wall-v2-sprites", out, "0b8474fa3b00d726674cc5c3f58b465274c55fcb1a59959051686885be1952e6")
}
