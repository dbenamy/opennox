...
func TestGameMessageClientSessionPlayerObjects(t *testing.T) {
			...
			for special := 0; special < 2; special++ {
				for _, frame := range []uint32{0, 100, 0xffffffff} {
					for status := 0; status < 256; status++ {
						...
						data = append(data, byte(status), byte(255-status), byte(13*status))
						...
						want.Anim = uint32(byte(13 * status))
						if want.Anim != 8 {
							want.Start = frame
						}
	...
	interactionCapture(t, "game-client-session-player-objects", rows)
}

func TestGameMessageClientSessionPlayerCreation(t *testing.T) {
	c, pix, env := newEffectsFullOwner(t)
	c.srv.NetList = netlist.New()
	c.srv.NetList.Init()
	t.Cleanup(c.srv.NetList.Free)
				...
				for _, code := range []uint16{0, 17} {
					for _, class := range []object.Class{0, 0x20400000} {
						for _, pos := range []image.Point{image.Pt(0, 0), image.Pt(5887, 5887), image.Pt(65535, 32768)} {
							...
							data = append(data, 0xf9, 253, 91)

							wantInput := bytes.Clone(data)
							cameraOnly := kind == 195 && wireCode == 0 && typeID == 0
							create := !cameraOnly && typeID != 0
							fail := !cameraOnly && typeID == 0
							...
							if create {
								...
								if kind == 195 && (r.Anim != 91 || r.Slave != 253 || dr.AnimDir != 8) {
	...
	interactionCapture(t, "game-client-session-player-creation", rows)
}
