...
func TestMapSectionsWallHistoricalRead(t *testing.T) {
	...
	var wants []expected
	for _, version := range []uint32{1, 3, 4, 5} {
				...
				for _, material := range []byte{0, 1, 2} {
					for _, direction := range []byte{0, 11} {
						for _, variation := range []byte{0, 3, 4, 255} {
							for _, point := range [][2]int{{8, 10}, {254, 252}} {
								...
								if version >= 2 {
									wire = append(wire, material, variation)
								}
								if version >= 4 {
	...
	spellbookCapture(t, "map-sections-wall-historical", out, "113c21fe58bd8ba04f97d01d6e94767956aabe7a36ade0b8d4321d0ab30bc5a8")
}
